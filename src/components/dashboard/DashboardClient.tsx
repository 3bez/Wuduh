...
import { dashboardCopy, type DashboardDict } from '@/lib/i18n/dashboard'
...
function formatDate(date: string, locale: Language) {
  // Use Gregorian Arabic month names + Arabic-Indic digits for `ar`.
...
export default function DashboardClient({ email, firstName, studies }: Props) {
  const { locale, dir, isRtl } = useLocale()
  const d = dashboardCopy[locale]
  const displayFont = isRtl ? 'var(--font-arabic), sans-serif' : 'var(--font-display), serif'
  const name = firstName ?? d.founderFallback
  const hasStudies = studies.length > 0

  // Eyebrow + mono labels: Arabic script must not be uppercased or letter-spaced.
  const monoFont = isRtl ? 'var(--font-arabic), sans-serif' : 'var(--font-mono), monospace'
  const monoLetterSpacing = isRtl ? '0' : '0.12em'
  const monoTransform = isRtl ? 'none' : 'uppercase'

  return (
    <div dir={dir} lang={locale} style={{ minHeight: '100vh', background: 'var(--bg-page)' }}>
      <style>{`
      ...
      <header style={{ background: 'var(--bg-surface)', borderBottom: '1px solid var(--border-default)', position: 'sticky', top: 0, zIndex: 50 }}>
        <div className="db-header-inner" style={{ maxWidth: 1120, margin: '0 auto', padding: '0 24px', height: 60, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            <span className="db-email" style={{ fontSize: 13, color: 'var(--text-faint)' }} dir="ltr">{email}</span>
      ...
      <main style={{ maxWidth: 1120, margin: '0 auto', padding: '48px 24px 80px' }}>
        <div style={{ marginBottom: 40 }}>
          <p style={{ fontFamily: monoFont, fontSize: 11, letterSpacing: monoLetterSpacing, textTransform: monoTransform, color: 'var(--gold-500)', marginBottom: 10 }}>{d.eyebrow}</p>
          <div className="dash-page-header">
            <div>
              <h1 className="db-heading" style={{ fontFamily: displayFont, fontSize: 30, fontWeight: 500, color: 'var(--text-primary)', letterSpacing: isRtl ? 0 : '-0.02em', lineHeight: 1.15, marginBottom: 6 }}>
                {d.welcome(name)}
              </h1>
              <p style={{ fontSize: 15, color: 'var(--text-muted)', lineHeight: 1.5 }}>
                {hasStudies ? d.subHasStudies : d.subEmpty}
              </p>
            </div>
        ...
        {hasStudies ? (
          <div className="study-grid">
            {studies.map(study => (
              <StudyCard key={study.id} study={study} d={d} locale={locale} isRtl={isRtl} />
            ))}
            <NewStudyCard d={d} />
          </div>
        ) : (
          <EmptyState d={d} locale={locale} isRtl={isRtl} displayFont={displayFont} monoFont={monoFont} monoLetterSpacing={monoLetterSpacing} monoTransform={monoTransform} />
        )}
      </main>
    </div>
  )
...
function StudyCard({ study, d, locale, isRtl }: { study: DashboardStudy; d: DashboardDict; locale: Language; isRtl: boolean }) {
  const pct = study.completionPercentage ?? 0
  const statusLabel = pct === 100 ? d.statusComplete : pct === 0 ? d.statusNotStarted : d.statusInProgress
  ...
  const langStyle = study.language === 'ar'
    ...
    : { background: 'var(--bg-subtle)', color: 'var(--text-faint)' }
  const badgeFont = isRtl ? 'var(--font-arabic), sans-serif' : 'var(--font-mono), monospace'
  const badgeSpacing = isRtl ? '0' : '0.08em'
    ...
    <div className="study-card" style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-default)', borderRadius: 16, padding: 24, display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <span style={{ fontFamily: badgeFont, fontSize: 10, letterSpacing: badgeSpacing, textTransform: 'uppercase', padding: '3px 8px', borderRadius: 99, ...langStyle }}>
            {study.language === 'ar' ? 'عربي' : 'EN'}
          </span>
          <span style={{ fontFamily: badgeFont, fontSize: 10, letterSpacing: badgeSpacing, padding: '3px 8px', borderRadius: 99, ...statusStyle }}>
            {statusLabel}
          </span>
        </div>
      </div>

      <RenameStudy studyId={study.id} currentName={study.startupName ?? null} untitledLabel={d.untitledStudy} tooltip={d.renameTooltip} />
      <p style={{ fontSize: 12, color: 'var(--text-faint)', marginBottom: 20 }}>{d.updated(formatDate(study.updatedAt, locale))}</p>

      <div style={{ marginBottom: 20, marginTop: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 7 }}>
          <span style={{ fontSize: 12, color: 'var(--text-faint)' }}>{d.progress}</span>
          <span style={{ fontFamily: badgeFont, fontSize: 12, color: pct === 100 ? 'var(--teal-500)' : 'var(--text-muted)', fontWeight: pct === 100 ? 600 : 400 }}>{localizeDigits(pct, locale)}%</span>
        </div>
...
function NewStudyCard({ d }: { d: DashboardDict }) {
  return (
    <Link href="/study/new" className="new-study-card" style={{ background: 'transparent', border: '1.5px dashed var(--border-strong)', borderRadius: 16, padding: 24, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12, textDecoration: 'none', minHeight: 220, transition: 'border-color 200ms, background 200ms' }}>
      ...
      <div style={{ textAlign: 'center' }}>
        <p style={{ fontSize: 14, fontWeight: 500, color: 'var(--text-secondary)', marginBottom: 3 }}>{d.newStudyCardTitle}</p>
        <p style={{ fontSize: 12, color: 'var(--text-faint)' }}>{d.newStudyCardSub}</p>
      </div>
    </Link>
  )
}

function EmptyState({ d, locale, isRtl, displayFont, monoFont, monoLetterSpacing, monoTransform }: { d: DashboardDict; locale: Language; isRtl: boolean; displayFont: string; monoFont: string; monoLetterSpacing: string; monoTransform: string }) {
  return (
    <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-default)', borderRadius: 20, padding: '72px 40px', textAlign: 'center', maxWidth: 560, margin: '0 auto' }}>
      ...
      <h2 style={{ fontFamily: displayFont, fontSize: 22, fontWeight: 500, color: 'var(--text-primary)', letterSpacing: isRtl ? 0 : '-0.015em', marginBottom: 10 }}>{d.emptyTitle}</h2>
      ...
      <div style={{ display: 'flex', marginBottom: 32, background: 'var(--bg-subtle)', borderRadius: 12, overflow: 'hidden' }}>
        {d.emptyStats.map((s, i) => (
          <div key={s.label} style={{ flex: 1, padding: '16px 8px', textAlign: 'center', borderInlineEnd: i < 2 ? '1px solid var(--border-default)' : undefined }}>
            <div style={{ fontFamily: displayFont, fontSize: 24, fontWeight: 500, color: 'var(--text-primary)', letterSpacing: isRtl ? 0 : '-0.02em', lineHeight: 1, marginBottom: 4 }}>{localizeDigits(s.num, locale)}<span style={{ color: 'var(--gold-500)' }}>.</span></div>
            <div style={{ fontSize: 11, color: 'var(--text-faint)', textTransform: monoTransform, letterSpacing: isRtl ? '0' : '0.06em', fontFamily: monoFont }}>{s.label}</div>
          </div>
        ))}
      </div>
