...
export default function RenameStudy({ studyId, currentName, untitledLabel = 'Untitled study', tooltip = 'Click to rename' }: Props) {
  ...
  async function save() {
    ...
    if (!trimmed || trimmed === currentName) { setValue(currentName ?? ''); setEditing(false); return }
    setSaving(true)
    await fetch(`/api/studies/${studyId}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ startup_name: trimmed }),
    })
  ...
  if (editing) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
        <input ref={inputRef} value={value} onChange={e => setValue(e.target.value)} onKeyDown={handleKeyDown} onBlur={save} disabled={saving} maxLength={80}
          style={{ fontFamily: 'var(--font-display), serif', fontSize: 17, fontWeight: 500, color: 'var(--text-primary)', background: 'var(--bg-subtle)', border: '1.5px solid var(--gold-500)', borderRadius: 7, padding: '3px 8px', outline: 'none', width: '100%', letterSpacing: '-0.01em', boxShadow: '0 0 0 3px rgba(201,168,76,0.12)' }} />
        {saving && <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="var(--gold-500)" strokeWidth="2.5" strokeLinecap="round" style={{ flexShrink: 0, animation: 'spin 0.8s linear infinite' }}><style>{'@keyframes spin{to{transform:rotate(360deg)}}'}</style><path d="M21 12a9 9 0 1 1-6.219-8.56" /></svg>}
      </div>
    )
  }
