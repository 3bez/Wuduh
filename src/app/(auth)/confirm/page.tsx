...
export default function ConfirmPage() {
  ...
  const t = authCopy[locale].confirm
  ...
  useEffect(() => {
    ...
    authClient.getSession().then(({ data }) => {
      if (data?.session) {
        ...
        setState('error')
        setMessage(t.errorExpiredMessage)
      }
    })
  }, [router, t.errorExpiredMessage])
        ...
        <div className="confirm-card" style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-default)', borderRadius: 20, padding: '48px 36px' }}>
          {state === 'verifying' && (
            <>
              ...
              <h2 style={{ fontFamily: isRtl ? 'var(--font-arabic), sans-serif' : 'var(--font-display), serif', fontSize: 20, fontWeight: 500, color: 'var(--text-primary)', marginBottom: 8 }}>{t.verifyingHeading}</h2>
              <p style={{ fontSize: 14, color: 'var(--text-muted)', lineHeight: 1.6, fontFamily: isRtl ? 'var(--font-arabic), sans-serif' : undefined }}>{t.verifyingSub}</p>
            </>
          )}
          ...
          {state === 'success' && (
            <>
              ...
              <p style={{ fontSize: 14, color: 'var(--text-muted)', lineHeight: 1.65, marginBottom: 24, fontFamily: isRtl ? 'var(--font-arabic), sans-serif' : undefined }}>{t.successBody}</p>
          ...
          {state === 'error' && (
            <>
              ...
              <h2 style={{ fontFamily: isRtl ? 'var(--font-arabic), sans-serif' : 'var(--font-display), serif', fontSize: 20, fontWeight: 500, color: 'var(--text-primary)', marginBottom: 10 }}>{t.errorHeading}</h2>
              {message && <p style={{ fontFamily: 'var(--font-mono), monospace', fontSize: 12, color: 'var(--danger-500)', marginBottom: 16 }}>{message}</p>}
              <p style={{ fontSize: 14, color: 'var(--text-muted)', lineHeight: 1.65, marginBottom: 28, fontFamily: isRtl ? 'var(--font-arabic), sans-serif' : undefined }}>{t.errorBody}</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <Link href="/signup" style={{ display: 'block', background: 'var(--text-primary)', color: 'var(--bg-page)', borderRadius: 9, padding: '12px 0', fontSize: 14, fontWeight: 500, textDecoration: 'none', textAlign: 'center', fontFamily: isRtl ? 'var(--font-arabic), sans-serif' : undefined }}>{t.errorRequestNew}</Link>
                <Link href="/login" style={{ display: 'block', background: 'transparent', border: '1.5px solid var(--border-default)', color: 'var(--text-muted)', borderRadius: 9, padding: '11px 0', fontSize: 14, textDecoration: 'none', textAlign: 'center', fontFamily: isRtl ? 'var(--font-arabic), sans-serif' : undefined }}>{t.errorBackToSignIn}</Link>
              </div>
            </>
          )}
        </div>

        <p style={{ fontSize: 12, color: 'var(--text-hint)', marginTop: 24, fontFamily: isRtl ? 'var(--font-arabic), sans-serif' : undefined }}>
          {t.needHelp} <Link href="/#contact" style={{ color: 'var(--gold-500)', textDecoration: 'none' }}>{t.contactUs}</Link>
        </p>
      </div>
    </div>
  )
