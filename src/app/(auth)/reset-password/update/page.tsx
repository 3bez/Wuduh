...
export default function UpdatePasswordPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Sign out all other sessions and redirect to dashboard after brief pause
    setTimeout(async () => {
  ...
  return (
    <div style={{
      ...
      <style>{`
        ...
        .up-show:hover { color: var(--text-faint); }
          ...
          {done ? (
            ...
            <>
              ...
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  ...
                  <div style={{ position: 'relative' }}>
                    <input
                      ...
                      className="up-input" autoFocus
                      value={password} onChange={e => setPassword(e.target.value)}
                      placeholder="At least 8 characters"
                      style={{ paddingRight: 44 }}
                    />
                    <button
                      type="button" className="up-show" onClick={() => setShowPass(v => !v)}
                ...
                {/* Confirm password */}
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  <label htmlFor="confirm" style={{ fontSize: 13, fontWeight: 500, color: 'var(--text-secondary)' }}>
                    Confirm password
                  </label>
                  <input
                    ...
                    style={{
                      borderColor: confirm && confirm !== password ? 'var(--danger-500)' : undefined,
                    }}
                  />
                  {confirm && confirm !== password && (
                    <p style={{ fontSize: 12, color: 'var(--danger-500)', fontFamily: 'var(--font-mono), monospace' }}>
                      Passwords do not match
                    </p>
                  )}
                  {confirm && confirm === password && (
                    <p style={{ fontSize: 12, color: 'var(--teal-500)', fontFamily: 'var(--font-mono), monospace' }}>
                      ✓ Passwords match
                    </p>
                  )}
                </div>
