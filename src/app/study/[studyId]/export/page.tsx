...
// Mini document preview — mirrors the paper mockup from the landing page
function DocumentPreview({ name, pct, lang }: { name: string | null; pct: number; lang: 'en' | 'ar' }) {
  ...
  return (
    <div style={{
      background: '#FAF7F0',
      borderRadius: 8,
      boxShadow: '0 12px 32px rgba(13,27,42,0.12), 0 2px 8px rgba(13,27,42,0.06)',
      overflow: 'hidden',
      position: 'relative',
    }}>
      {/* Geometric net watermark */}
      ...
      <div style={{ position: 'relative', padding: '20px 24px' }}>
        ...
        <div style={{
          ...
          marginBottom: 14, paddingBottom: 12, borderBottom: '1px solid #EAE3D2',
        }}>
        ...
        <div style={{
          ...
          color: '#A6852F', marginBottom: 5,
        }}>
          {lang === 'ar' ? new Date().toLocaleDateString('ar-SA', { month: 'long', year: 'numeric' })
            : new Date().toLocaleDateString('en-GB', { month: 'long', year: 'numeric' })}
        </div>
        <div style={{
          fontFamily: lang === 'ar' ? 'var(--font-arabic), sans-serif' : 'var(--font-display), serif',
          fontSize: 14, fontWeight: 600, color: '#0D1B2A', lineHeight: 1.3, marginBottom: 4,
        }}>
        ...
        {sections.map((s, i) => (
          <div key={i} style={{ marginBottom: 10 }}>
            <div style={{
              ...
              fontSize: 8, fontWeight: 600, color: '#0D1B2A', marginBottom: 5,
            ...
            {[85, 100, 62].slice(0, i === 2 ? 3 : 2).map((w, j) => (
              <div key={j} style={{
                ...
                background: i * 2 + j < Math.round(pct / 12) ? '#D4DBE3' : '#EAE3D2',
                width: `${w}%`, marginBottom: 3,
              }} />
            ))}
          </div>
        ))}
  ...
  if (loading) {
    return (
      <div style={{ minHeight: '100vh', background: '#F4F6F8', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C9A84C" strokeWidth="2" strokeLinecap="round" style={{ animation: 'spin 0.8s linear infinite' }}>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F4F6F8' }} dir={dir}>
      <style>{`
        ...
        .ep-accent:hover { background: #D6BC72 !important; }
        .ep-ghost:hover { background: #E8ECF1 !important; }
      ...
      <main style={{ maxWidth: 960, margin: '0 auto', padding: '48px 20px 80px' }}>
        <div style={{
          ...
          <div>
            ...
            <h1 style={{
              fontFamily: lang === 'ar' ? 'var(--font-arabic), sans-serif' : 'var(--font-display), serif',
              fontSize: 28, fontWeight: 500, color: '#0D1B2A',
              letterSpacing: lang === 'ar' ? 0 : '-0.02em', lineHeight: 1.2,
              marginBottom: 8,
            }}>
              {lang === 'ar' ? 'دراستك جاهزة للتصدير' : 'Your study is ready to export'}
            </h1>
            <p style={{ fontSize: 14, color: '#647183', lineHeight: 1.65, marginBottom: 28 }}>
              {lang === 'ar'
                ? 'سيتم إنشاء ملف PDF احترافي جاهز للمستثمرين من إجاباتك. يستغرق هذا حتى 15 ثانية.'
                : 'A professional, investor-ready PDF will be generated from your answers. This takes up to 15 seconds.'}
            </p>
            ...
            <div style={{
              ...
              <div style={{
                ...
                <span style={{ fontSize: 13, color: '#36404D', fontWeight: 500 }}>
                  {lang === 'ar' ? 'اكتمال الدراسة' : 'Study completion'}
                </span>
                <span style={{
                  fontFamily: 'var(--font-mono), monospace',
                  fontSize: 13,
                  color: pct === 100 ? '#0D9488' : '#C9A84C',
                  fontWeight: 600,
                }}>
                  {pct}%
                </span>
              </div>
              <div style={{ height: 5, background: '#F4F6F8', borderRadius: 99, overflow: 'hidden' }}>
                <div style={{
              ...
              {pct < 100 && (
                <p style={{ fontSize: 12, color: '#B4811E', marginTop: 10 }}>
                  {lang === 'ar'
                    ? `${100 - pct}% من البطاقات الإلزامية لم تكتمل — ستُعلَّم الأقسام الناقصة في PDF.`
                    : `${100 - pct}% of mandatory cards are incomplete — missing sections will be flagged in the PDF.`}
                </p>
              )}
            </div>
            ...
            <div style={{ marginBottom: 28 }}>
              {[
                ...
                lang === 'ar' ? `RTL عربي أو LTR إنجليزي — يتطابق مع لغة دراستك` : `${lang === 'ar' ? 'Arabic RTL' : 'English LTR'} — matches your study language`,
              ].map((item, i) => (
                <div key={i} style={{
                  ...
                  <div style={{
                    ...
                    background: '#DCEFE6', display: 'flex', alignItems: 'center', justifyContent: 'center',
                  }}>
                    <svg width="9" height="9" viewBox="0 0 12 12" fill="none" stroke="#0D9488" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                      <polyline points="2 6 5 9 10 3" />
                    </svg>
                  </div>
                  <span style={{ fontSize: 13, color: '#4A5666', lineHeight: 1.5 }}>{item}</span>
                </div>
              ))}
            </div>
              ...
              <div style={{
                ...
                <div style={{ position: 'relative', width: 48, height: 48 }}>
                  <svg width="48" height="48" viewBox="0 0 48 48" style={{ position: 'absolute', inset: 0 }}>
                    <circle cx="24" cy="24" r="20" fill="none" stroke="#F4F6F8" strokeWidth="3" />
                ...
                <div style={{ textAlign: 'center' }}>
                  <p style={{
                    fontFamily: lang === 'ar' ? 'var(--font-arabic), sans-serif' : 'var(--font-display), serif',
                    fontSize: 16, fontWeight: 500, color: '#0D1B2A', marginBottom: 5,
                  }}>
            ...
            {state === 'done' && url && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <div style={{
                  background: '#DCEFE6', border: '1px solid #A9E0D9',
                  ...
                  <div style={{
                    ...
                    <svg width="14" height="14" viewBox="0 0 12 12" fill="none" stroke="#0D9488" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                      <polyline points="2 6 5 9 10 3" />
                    </svg>
                  </div>
                  <div style={{ textAlign: dir === 'rtl' ? 'right' : 'left' }}>
                    <p style={{ fontSize: 13, fontWeight: 600, color: '#11724E', marginBottom: 1 }}>
                      {lang === 'ar' ? 'PDF جاهز!' : 'PDF ready!'}
                    </p>
                    <p style={{ fontSize: 12, color: '#15805A' }}>
                      {lang === 'ar' ? 'الرابط صالح لمدة ساعة واحدة.' : 'Link is valid for 1 hour.'}
                    </p>
                  </div>
                </div>
                <a
            ...
            {state === 'error' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <div style={{
                  background: '#F6E0DA', border: '1px solid #F0C4B3',
                  borderRadius: 10, padding: '14px 18px',
                }}>
                  <p style={{ fontSize: 13, fontWeight: 600, color: '#A53D27', marginBottom: 3 }}>
                    {lang === 'ar' ? 'فشل الإنشاء' : 'Generation failed'}
                  </p>
                  {error && <p style={{ fontSize: 12, color: '#C0492F', fontFamily: 'var(--font-mono), monospace' }}>{error}</p>}
                </div>
                <button
            ...
            <div style={{ marginTop: 20, textAlign: dir === 'rtl' ? 'right' : 'left' }}>
              <Link
                ...
                className="ep-back"
                style={{ fontSize: 13, color: '#8795A6', textDecoration: 'none', transition: 'color 140ms' }}
              >
